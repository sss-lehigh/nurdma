...
void rc_resources::initOpenClosed(size_t n_machines, size_t n_workers,
                                  const char *rqstd_servername,
                                  size_t n_clients, const char *rqstd_dev_name,
                                  int rqstd_ib_port, size_t rqstd_obj_size,
                                  size_t rqstd_buf_size, int rqstd_numa,
                                  int rqstd_tcp_port) {
  ...
  // setup for synchronization after initializing each thread
  pthread_barrier_init(&client_barrier, NULL, (2 * num_clients) + 1); // wait for both clients and polling threads (and launching thread)
  pthread_barrier_init(&worker_barrier, NULL, num_workers + 1); // wait for workers and launching thread
  go = new std::atomic<bool>(false); // flag to wait until both client and server are initialized

  // flags to coordinate threads
  for (int i = 0; i < num_clients; ++i) client_flags.push_back(new bool(false));
  for (int i = 0; i < num_workers; ++i) worker_flags.push_back(new bool(false));

  // prefilled random numbers for each thread
  for (int i = 0; i < num_clients; ++i) client_rands.push_back(new std::vector<int>());
  for (int i = 0; i < num_workers; ++i) worker_rands.push_back(new std::vector<int>());
  ...
  for (auto &cq : cqs) enforce_numa_zone(cq, numa);
  enforce_numa_zone(&qps, numa);
  for (auto &qp : qps) enforce_numa_zone(qp, numa);
  enforce_numa_zone(&client_flags, numa);
  for (auto &f : client_flags) enforce_numa_zone(f, numa);
  enforce_numa_zone(&worker_flags, numa);
  for (auto &f : worker_flags) enforce_numa_zone(f, numa);
  enforce_numa_zone(&client_rands, numa);
  for (auto &r : client_rands) enforce_numa_zone(r, numa);
  enforce_numa_zone(&worker_rands, numa);
  for (auto &r : worker_rands) enforce_numa_zone(r, numa);
...
int rc_resources::setup_tcp_socket(std::string servername, int port) {
  ...
  if (!servername.empty()) {
    ...
    if (sock < 0) {
      std::cerr << "failed to establish TCP connection to server " << servername
                << ", port " << port << std::endl;
    }
  } else {
    std::cout << "waiting on port " << port << " for TCP connection"
              << std::endl;
    sock = sock_connect(nullptr, port);
    if (sock < 0) {
      std::cerr << "failed to establish TCP connection with client on port "
                << port << std::endl;
    }
  }
  return sock;
...
int rc_resources::setup_ib_dev() {
  ...
  for (i = 0; i < num_devices; ++i) {
    if (dev_name.empty()) {  // use first device found if no device requested
      ...
      std::cerr << "device not specified, using first one found: " << dev_name
                << std::endl;
    }
...
int rc_resources::setup_shmem_buf() {
  ...
  shmem_buf_size = shmem_buf_size > num_clients * obj_size
                       ? shmem_buf_size
                       : num_clients * obj_size;

  // what is true difference between numa_alloc and malloc -> move
#if BINDNUMA
  // shmem_buf = (unsigned char *)numa_alloc(shmem_buf_size);
  shmem_buf = (unsigned char *)aligned_alloc(CACHELINE_SIZE, shmem_buf_size);
  ...
  if (!shmem_buf) {
    std::cerr << "fail to allocate " << shmem_buf_size << " bytes on numa zone "
              << numa << std::endl;
    rc = 1;
  ...
  mr_flags = static_cast<ibv_access_flags>(
...
int rc_resources::setup_qps() {
  ...
  for (i = 0; i < num_clients; ++i) {
    ...
    while (temp_cqptr == nullptr) {
      temp_cqptr = ibv_create_cq(ib_ctx, num_cqe, nullptr, nullptr,
                                 0);  // create cq that can handle max cqes
  ...
  std::cerr << "Real Send QP WR maximum: " << qp_init_attr.cap.max_send_wr
            << std::endl;
  std::cerr << "Real Send QP SGE maximum: " << qp_init_attr.cap.max_send_sge
            << std::endl;
...
int rc_resources::modify_qp_to_rtr(struct ibv_qp *qp, uint32_t remote_qpn,
                                   uint16_t dlid) {
  struct ibv_qp_attr attr = {};
  int flags;
  int rc;
  ...
  rc = ibv_modify_qp(qp, &attr, flags);
  if (rc)
    std::cerr << "failed to modify QP state to RTR - " << strerror(rc)
              << std::endl;
  return rc;
}
...
int rc_resources::modify_qp_to_rts(struct ibv_qp *qp) {
  ...
  rc = ibv_modify_qp(qp, &attr, flags);
  if (rc) std::cerr << "failed to modify QP state to RTS" << std::endl;
  return rc;
}
...
int rc_resources::connectQPs() {
  ...
  for (i = 0; i < num_clients; ++i) {
    ...
    if (sock_sync_data(sock, sizeof(struct cm_con_data_t), (char *)&local_con,
                       (char *)&tmp_con) < 0) {
    ...
    if (modify_qp_to_init(qps[i])) return 1;
    if (modify_qp_to_rtr(qps[i], remote_con.qp_num, remote_con.lid)) return 1;
    if (modify_qp_to_rts(qps[i])) return 1;
...
void rc_resources::client_openloop(int client_id, int iters, int postlist,
  ...
  // flag to alert polling thread that client is finished
  volatile bool* done = client_flags[client_id];
  ...
  if (mode == 2) std::srand((uint)client_id);

  // initialize random numbers for each iteration
  std::vector<int> rands = *(client_rands[client_id]);
  for (int i = 0; i < iters; ++i) {
    rands.push_back(std::rand());
  }

  // wait for other clients and all polling threads
  rc = pthread_barrier_wait(&client_barrier);
  if (rc != PTHREAD_BARRIER_SERIAL_THREAD && rc != 0) {
    std::cerr << "pthread_barrier_wait failed with error code: " << strerror(rc) << std::endl;
    exit(1);
  }

  while(!go->load()); // wait until launching thread has synced with server
  ...
  for (i = 0; i < todo; ++i) {
    ...
    switch (mode) {
      ...
      case 2:  // choose read/write based on FB's ETC workload
        r = rands[i];
        if (r < RAND_MAX * write_ratio)
          opcode = 1;
        else
          opcode = 0;
        if (opcode == 1) memset(shmem_buf + offset, 0, obj_size);
        break;
    ...
    for (j = 0; j < postlist; ++j) {
      srs[j].send_flags =
          (((i * postlist) + j + 1) % sendbatch == 0 ? IBV_SEND_SIGNALED : 0);
    ...
    if (rc) {
      std::cerr << "Client " << client_id << " failed iteration " << i
                << " with code " << rc << " (trying again)" << std::endl;
    }

    if (think > 0) {
      // think is number of nanoseconds to wait
      clock_gettime(CLOCK_REALTIME, &t1);
      do {
        clock_gettime(CLOCK_REALTIME, &t2);
      } while ((BILLION * (t2.tv_sec - t1.tv_sec) + t2.tv_nsec - t1.tv_nsec) < think);
    }
  }
  fprintf(stderr, "%d, total failures: %d\n", x + y, failures);

// wait for polling thread to finish clearing completion queue
client_openloop_exit:
  *done = true;
  while (*done); // spin until the polling thread finishes
}

/**
 * @brief Polls the CQE associated with a given client to monitor for completions.
 *        Periodically outputs the instantaneous throughput until a flag is set
 *        by the client indicating that the run is over.
 * 
 * @param client_id 
 * @param iters 
 * @param sendbatch 
...
void rc_resources::poll_cq(int client_id, int iters, int sendbatch) {
  ...
  volatile bool *done; // ensures loads and stores to done are visible by clients
  int poll_result, rc;
  ...
  // wait for client threads to initialize random number before continuing
  clientWait();
  while(!go->load()); // wait until launching thread syncs with server
  ...
  do {
    ...
    if (poll_result < 0) {
      /* poll CQ failed */
      std::cerr << "poll CQ failed with error " << wc.status << "("
      ...
      if (*done) // if client finished and there are no CQEs remaining, terminate
        finished = true;
    }
    clock_gettime(CLOCK_REALTIME, &t2);
  ...
  } while ((total_received < total_expected) || !finished);
  *done = false; // set done back to false to alert client that polling is complete
  std::cerr << "CQE received: " << total_received << std::endl;
}

std::vector<double> rc_resources::client_closedloop(int client_id, int iters,
                                                    int postlist, int sendbatch,
                                                    int mode, int think, float write_ratio) {
  /* send variables */
  int ops[] = {IBV_WR_RDMA_READ, IBV_WR_RDMA_WRITE, IBV_WR_ATOMIC_CMP_AND_SWP,
               IBV_WR_ATOMIC_FETCH_AND_ADD};
  int offset, opcode;
  ...
  for (i = 0; i < todo; ++i) {
    // // designed to test QP replacement 
    // if (client_id == 1 && i > todo/100) {
    //   break;
    // } // we expect to see the average latency of a client_id >= 7 to become faster
    ...
    _elapsed_iter = (uint64_t)(BILLION * (end.tv_sec - t2.tv_sec) +
                               end.tv_nsec - t2.tv_nsec);
    _elapsed_tot += _elapsed_iter;
    _elapsed[i] = (double)_elapsed_iter;

    if (think > 0) {
      // int think_ns = think * 1000; // think is given in microseconds
      clock_gettime(CLOCK_REALTIME, &t1);
      do {
        clock_gettime(CLOCK_REALTIME, &t2);
      } while ((BILLION * (t2.tv_sec - t1.tv_sec) + t2.tv_nsec - t1.tv_nsec) < think);
  ...
  fprintf(stderr, "%d, total failures: %d\n", x + y, failures);
  elapsed_avg = _elapsed_tot / (double)(iters); // (client_id != 1 ? iters : iters/100)
  elapsed_stddev = 0.0;
  for (i = 0; i < iters; ++i) {
...
void rc_resources::worker(int worker_id, int mode, int think, int timeout,
  ...
  // initialize random numbers for each iteration
  std::vector<int> rands = *(worker_rands[worker_id]);
  ...
  for (i = 0; !*(worker_flags[worker_id]); ++i) {
    switch (mode) {
      case 0:
        temp += vol_shmem_buf[(obj_size * worker_id +
                              rands[i % RANDS] + i) %
                              shmem_buf_size];
        break;
      case 1:  // write as uint64_t for consistency with CAS
        //*reinterpret_cast<uint64_t *>(&shmem_buf[(obj_size * worker_id + i *
        // sizeof(uint64_t)) % shmem_buf_size]) = (uint64_t) worker_id;
        vol_shmem_buf[(obj_size * worker_id + rands[i % RANDS] +
                       i) %
                      shmem_buf_size] = (unsigned char)worker_id;
        break;
      case 2:
        r = rands[i % RANDS];
        if (r < RAND_MAX * write_ratio)
          vol_shmem_buf[(obj_size * worker_id + i) % shmem_buf_size] =
              (unsigned char)worker_id;
        else
          temp += vol_shmem_buf[(obj_size * worker_id + i) % shmem_buf_size];
        break;
      case 3: {
        // uncomment the following line to atomically access on the cache line
        // boundary auto * atomic_buf = reinterpret_cast<std::atomic<unsigned
        // long> *>(&shmem_buf[(obj_size * worker_id + i * sizeof(uint64_t) +
        // sizeof(uint64_t) / 2) % (shmem_buf_size - sizeof(uint64_t))]);
        auto *atomic_buf = reinterpret_cast<std::atomic<unsigned long> *>(
            &shmem_buf[(obj_size * worker_id + i * sizeof(uint64_t)) %
                       (shmem_buf_size)]);
        ...
        do {
          //  try until succeed
        } while (!std::atomic_compare_exchange_strong(atomic_buf, &expected,
                                                      desired));
        break;
      }
      case 4: {
    ...
    if (think > 0) {
      ...
      do {
        clock_gettime(CLOCK_REALTIME, &t1);
        diff = (BILLION * (t1.tv_sec - t2.tv_sec) + t1.tv_nsec - t2.tv_nsec);
      } while (diff < think);
    }

#if WORKER_NUMA_CHK
    if (i % 1000000 ==
        0)  // periodically checks that everything is in the expected NUMA zone
      checkNumaLocality();
#endif
    ...
    if (i % 1000000 == 0) {
      ...
      std::cout << (BILLION * (t4.tv_sec - t3.tv_sec) + t4.tv_nsec -
                    t3.tv_nsec) /
                       (double)100000
                << std::endl;  // print worker latency
      clock_gettime(CLOCK_REALTIME, &t3);
    }
#endif

    clock_gettime(CLOCK_REALTIME, &t2);
    if (BILLION * (t2.tv_sec - start.tv_sec) + t2.tv_nsec - start.tv_nsec >=
        timeout * BILLION) {
      // printf("stopping...");
...
int rc_resources::getNumClients() { return static_cast<int>(num_clients); }

void rc_resources::signalWorkersDone() {
  for (auto &d : worker_flags) *d = true;
}
...
void rc_resources::prepare_requests(ibv_send_wr *srs, ibv_sge *sges, int tid,
                                    int mode, int offset, int postlist) {
  int i;
...
void rc_resources::start() {
  go->store(true);
}

void rc_resources::reinitClientBarrier(int count) {
  int err;
  if ((err = pthread_barrier_destroy(&client_barrier)) != 0) {
    std::cerr << "rc_resources::reinitClientBarrier: failed to destory current client barrier: " << strerror(err) << std::endl;
    exit(1);
  }
  if ((err = pthread_barrier_init(&client_barrier, NULL, count)) != 0) {
    std::cerr << "rc_resources::reinitClientBarrier: failed to initialize new client barrier: " << strerror(err) << std::endl;
    exit(1);
  }
}

void rc_resources::clientWait() {
  int err = pthread_barrier_wait(&client_barrier);
  if (err != PTHREAD_BARRIER_SERIAL_THREAD && err != 0) {
    std::cerr << "pthread_barrier_wait failed with error code: " << strerror(err) << std::endl;
    exit(1);
  }
}

void rc_resources::workerWait() {
  int err = pthread_barrier_wait(&worker_barrier);
  if (err != PTHREAD_BARRIER_SERIAL_THREAD && err != 0) {
    std::cerr << "pthread_barrier_wait failed with error code: " << strerror(err) << std::endl;
    exit(1);
  }
