...
def plot_speedup(cread_file_list, cwrite_file_list, cread_label_map, cwrite_label_map, cread_data, cwrite_data, numa_zone):
    ...
    for cread_file, cwrite_file in zip(cread_file_list, cwrite_file_list):
        ...
        plt.bar((position) + 0 * width, cread_speedup, width, color='red', edgecolor='black', hatch="\\\\", label="read Speedup", zorder=2)
        plt.bar((position) + 1 * width, cwrite_speedup, width, color='yellow', edgecolor='black', hatch="--", label="write speedup", zorder=3)
        position += 1
...
if __name__ == "__main__":
    ...
    plot_speedup(sorted(cread_file_list, key=byNumClients), sorted(cwrite_file_list, key=byNumClients),
